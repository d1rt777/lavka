...
import { isYarnPnP, assert, assertPosixPath, isViteServerSide, isAbsolutePath, toPosixPath, projectInfo, objectAssign, joinEnglish, findRollupBundleEntry, assertUsage, isNotNullish, injectRollupInputs, normalizeRollupInput, deepEqual, escapeRegex, getGlobalObject, genRandomId, } from './utils.js';
...
function clearAutoImporter(status, skipWrite) {
...
function findServerEntry(bundle, outDir) {
    const entry = findRollupBundleEntry(serverEntryFileNameBaseAlternative, bundle, outDir) ||
        findRollupBundleEntry(serverEntryFileNameBase, bundle, outDir);
    assertUsage(entry, errMsgEntryRemoved([
        //
...
function getServerEntryName(config) {
    ...
    assert(entries[serverEntryFileNameBase] !== serverEntryVirtualId &&
...
function writeAutoImporterFile(fileContentNew, skipWrite) {
    // Only write if necessary
    {
        let fileContentCurrent = null;
        try {
        ...
        if (fileContentCurrent !== null) {
            if (fileContentCurrent === fileContentNew)
                return;
            if (skipWrite?.(fileContentCurrent))
                return;
        }
    }
