...
function shortenId(id, root) {
	const normalized = id.replaceAll("\\", "/");
	const normalizedRoot = root.replaceAll("\\", "/");
	const nmIdx = normalized.lastIndexOf("/node_modules/");
	if (nmIdx !== -1) {
		const [pkg, scope] = normalized.slice(nmIdx + 14).split("/");
	...
	return {
		...
		config: {
			...
			handler() {
				...
				if (this.meta?.rolldownVersion) buildEnvOptions.rolldownOptions = { input: { index: entry } };
				else buildEnvOptions.rollupOptions = { input: { index: entry } };
				return { environments: { ssr: { build: { ...buildEnvOptions } } } };
			}
		},
		async buildStart() {
			const resolved = await this.resolve(entry);
			if (resolved) resolvedEntry = resolved.id;
		},
		transform(code, id) {
			if (resolvedEntry && id === resolvedEntry) {
				if (!code.includes("virtual:ud:")) this.warn(`{ entry: "${entry}" } is missing "virtual:ud:catch-all" import.`);
			}
		}
	};
}
...
function auto(options) {
	...
	return [
		catchAll(),
		devServer(),
		...options?.entry ? [target(options.entry)] : [...node$1(options?.node).map((p) => {
			p[INSTANCE] = instance;
...
function generateCode(entries, compiledFindRoute) {
	const eagerEntries = entries.filter((e) => e.eager);
	const staticImports = eagerEntries.map((e) => `import __eager_${e.key} from ${JSON.stringify(e.resolvedId)};`).join("\n");
	const mapEntries = entries.map((e) => e.eager ? `  "${e.key}": () => Promise.resolve({ default: __eager_${e.key} })` : `  "${e.key}": () => import(${JSON.stringify(e.resolvedId)})`).join(",\n");
		...
		load: {
			...
			async handler() {
				...
				for (const meta of getAllEntries()) {
					...
					if (existing) {
						let added = false;
						for (const route of routes) if (!existing.routes.has(route)) {
							added = true;
							existing.routes.add(route);
							for (const method of methods) addRoute(router, method, route, existing.key);
						}
						if (!added) duplicates.add(resolved.id);
						if (routes.has("/**")) existing.eager = true;
					} else {
						const entry = {
						...
						for (const route of routes) for (const method of methods) addRoute(router, method, route, entry.key);
					}
				}
